...
#         asset_weights = self.action(action)
#         obs, reward, done, truncated, info = self.env.step(asset_weights)
        
#         # 关键：必须把权重传出来，Callback 才能读到
...
class MetaQPSolver:
    """
    修改版 Solver：风险中性 (Risk Neutral)
    不再考虑协方差矩阵，完全依据预期收益率 (Mu) 进行分配。
    这会让组合变得非常激进 (Winner-Take-All)。
    """
    def __init__(self, n_assets, risk_aversion=0.0): # <--- 默认改为 0
        self.n_assets = n_assets
        self.risk_aversion = risk_aversion

    def solve(self, blended_mu, covariance_matrix=None):
        """
        求解: Max w.T * mu
        s.t. sum(w) = 1, w >= 0
        
        注意：covariance_matrix 在这里被忽略了，或者系数为 0
        """
        w = cp.Variable(self.n_assets)
        
        # 目标函数：只最大化收益
        # 为了防止数值问题，我们加上极小的正则项 (1e-6 * sum_squares)，
        # 这样当 mu 相同时能保持平均分配，而不是随机选一个
        ret = w @ blended_mu
        regularization = cp.sum_squares(w) * 1e-6 
        
        # 如果你真的想移除保护，risk_aversion 必须是 0
        if self.risk_aversion > 1e-5:
            risk = cp.quad_form(w, covariance_matrix)
            objective = cp.Maximize(ret - self.risk_aversion * risk)
        else:
            # 纯收益模式
            objective = cp.Maximize(ret - regularization)
...
class MetaDMPOActionWrapper(gym.ActionWrapper):
    def __init__(self, env):
        super().__init__(env)
        # 核心修改：初始化时将 risk_aversion 设为 0
        self.solver = MetaQPSolver(env.n_assets, risk_aversion=0.0) 
        self.last_alpha_weights = np.zeros(env.n_alphas)

    def action(self, action):
        # 放大 SAC 的输出，让 Softmax 更尖锐 (Temperature Scaling)
        # SAC 输出通常在 [-1, 1]，乘 5 后变成 [-5, 5]
        # Softmax([-5, 5]) 的差异比 Softmax([-1, 1]) 大得多，更容易出现 0.99 vs 0.01
        scaling_factor = 5.0 
        scaled_action = action * scaling_factor
        
        # 1. Softmax
        exp_x = np.exp(scaled_action - np.max(scaled_action))
        alpha_weights = exp_x / exp_x.sum()
        ...
        # 3. 协方差矩阵 (现在 Solver 会忽略它，或者只起极小的正则化作用)
        cov_matrix = np.eye(env_core.n_assets) # 传个单位阵占位即可
