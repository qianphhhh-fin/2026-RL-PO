...
def main(args, cfg_env=None):
    ...
    for epoch in range(epochs):
        ...
        loss_pi_r = (ratio * data["adv_r"]).mean() #+ 0.01 * temp_distribution.entropy().mean()
        ...
        ratio = torch.exp(log_prob - data["log_prob"])
        act = policy.step(data["obs"], deterministic=False)[0]
        loss_pi_c = -(ratio * data["adv_c"]).mean() #+ 0.01 * temp_distribution.entropy().mean()
        ...
        ratio = torch.exp(log_prob - data["log_prob"])
        actual_improve_c = (-(ratio * data["adv_c"]).mean()).item() - loss_pi_c.item()
        final_kl = (
